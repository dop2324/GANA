...
<c:choose>
	...
	<c:when test="${menu.value.mnu_ty == 'page+board'}">	<c:set var="menuType" value="P+B" /></c:when>
...
d.add("${menu.value.mnu_code}"
		...
		, "javascript:setMenuCode('<c:out value="${menu.value.mnu_code}" />');"
		, "", "", "", "", ""
		, <c:out value="${menu.value.mnu_sttus}" />
		);
</c:forEach>
document.write(d);
d.aNodes.forEach((item, index) => {
	if(item.id == this.parm_mnuCode) {
		openTree(this.parm_mnuCode);
    	d.s(index);
    }
});
