...
<form id="commentFrm" name="commentFrm" method="post" action="<c:url value="${managerDir }/cms/board/comment/comment_process.do" />">
...
<c:if test="${boardMap.bod_delSttus == 0 }">
	...
	<div class="comment_write">
		<div class="comment_member">
			...
			<label for="con_pw">비밀번호</label>
			<input type="password" id="con_pw" name="con_pw" maxlength="32" autocomplete="new-password" value="" />
		</div>
		<div class="comment_txt2">
			...
			<textarea id="con_cn" name="con_cn"></textarea>
...
function comment(con_sn, cmd) {
	//비밀번호확인
	chk_password = "비밀번호를 입력하세요<br/>"
						+ "<input type=\"password\" id=\"con_password\" name=\"update_pw\" maxlength=\"32\" autocomplete=\"new-password\" class=\"inp_txt\" /> "
						+ "<span>"
						+ "<a href=\"#self\" class=\"bt1\" onclick=\"checkPassword("+con_sn+", "+cmd+")\">확인</a>"
						+ "<a href=\"#self\" class=\"bt2\" onclick=\"updateCancel("+con_sn+")\">취소</a>"
						+ "</span>";
	$(".commentContent").show();
	$(".updateComment").hide();
...
function checkPassword(con_sn, cmd) {	
	...
	$.ajax({
		type:"get"
		, url:"<c:out value="${managerDir }/cms/board/comment/check_password.do" />"
		, dataType:"json"
		, data: {
		...
		, success:function(data) {
			if(data.result != 0) {
				if(cmd == 16) {
					updateContent = "<input type=\"password\" name=\"update_pw\" class=\"inp_txt\" maxlength=\"32\" value=\""+$("#con_password").val()+"\" />"
									+ "<textarea id=\"update_content\" name=\"update_cn\">"+data.con_cn+"</textarea> "
									+ "<a href=\"#self\" class=\"bt2\" onclick=\"updateData("+con_sn+")\">의견쓰기</a>"
									+ "<a href=\"#self\" class=\"bt1\" onclick=\"updateCancel("+con_sn+")\">취소</a>";
...
$(function(){
	$("#con_cn").on("keydown", function () {	
		...
		if (remain >= 0) {
			$("#commentWordCnt").css('color','blue');
		} else {
			$("#commentWordCnt").css('color','red')
		}
	});
});
</script>