...
{
	public static void createThumbImage(String where, String subPath, String fileName, int width, int height) throws IOException {
		String srcfilePath = String.format("%s/%s/%s", where, subPath, fileName);
		thumbImage(srcfilePath, width, height);
	}
