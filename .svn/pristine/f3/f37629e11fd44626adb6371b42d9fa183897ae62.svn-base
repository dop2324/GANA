...
function setMenuCode(mnu_code, boardFlag) {
	if(boardFlag == 1) {
...
<c:set var="boardFlag" value="0" />
<c:choose>
	<c:when test="${menu.value.mnu_ty == 'menu+page'}">		<c:set var="menuType" value="P" /></c:when>
	<c:when test="${menu.value.mnu_ty == 'menu+program'}">	<c:set var="menuType" value="PG" /></c:when>
	<c:when test="${menu.value.mnu_ty == 'page'}">			<c:set var="menuType" value="P" /></c:when>
	<c:when test="${menu.value.mnu_ty == 'page+program'}">	<c:set var="menuType" value="P+PG" /></c:when>
	<c:when test="${menu.value.mnu_ty == 'board'}">			<c:set var="menuType" value="B" /><c:set var="boardFlag" value="1" /></c:when>
	<c:when test="${menu.value.mnu_ty == 'page+board'}">	<c:set var="menuType" value="P+B" /><c:set var="boardFlag" value="1" /></c:when>
	...
	--%>
	<c:when test="${menu.value.mnu_ty == 'shareBoard'}">	<c:set var="menuType" value="SB" /><c:set var="boardFlag" value="1" /></c:when>
</c:choose>
d.add("${menu.value.mnu_code}"
		...
		, "javascript:setMenuCode('<c:out value="${menu.value.mnu_code}" />', <c:out value="${boardFlag}" />);"		
		, "", "", "", "", ""
		, <c:out value="${boardFlag}" />
		);
</c:forEach>
document.write(d);
